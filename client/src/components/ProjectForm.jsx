...
const ProjectForm = ({data=[], onChange}) => {
  ...
  return (
   ...
   <button
   onClick={()=>removeProject(index)} className='text-red-500 hover:text-red-700 transition-colors'>
    <Trash2 className='size-4'/>
   </button>
     </div>
     <div className='grid gap-3'>
        <input  value={project.name || ""} type='text' onChange={(e)=>updateProject(index,"name",e.target.value)} 
        placeholder=' Project Name' className='px-3 py-2 text-sm  rounded-lg'/>

     <input  value={project.type || ""} type='text' onChange={(e)=>updateProject(index,"type",e.target.value)} 
